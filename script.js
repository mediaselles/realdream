(function () {
  ...
  function renderGallery() {
    ...
    grid.innerHTML = items.map(char => {
      ...
      const mediaHtml = char.video
        ? `<video class="lp-card-img lp-card-video" src="${escapeHtml(char.video)}" poster="${escapeHtml(char.image)}" autoplay loop muted playsinline preload="none"></video>`
        : `<img class="lp-card-img" src="${escapeHtml(char.image)}" alt="${escapeHtml(char.name)}" loading="lazy" onerror="this.onerror=null; this.src='${char.fallbackImage || ''}';">`;
  ...
  function openCharacterModal(charId) {
    ...
    const video = modal.querySelector('.char-modal-video');
  ...
  /* Video Viewport Auto-Play Observer */
  ...
  function setupCardVideoObserver() {
    ...
    document.querySelectorAll('.lp-card-video').forEach(vid => {
      cardVideoObserver.observe(vid);
    });
  }
